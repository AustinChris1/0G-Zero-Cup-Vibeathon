...
const GLYPHS = ["◆", "⬡", "✦", "◭", "❖", "⬢", "✕", "⌬", "▲", "●"];
// All agents run on the same live 0G Compute model (the available TEE provider);
// they differ by strategy, not base model. Keep the label honest.
const MODEL = process.env.OG_MODEL || RUNTIME_MODEL;
...
export async function GET() {
  ...
  const agents = await Promise.all(
    all.map(async (a) => ({ agent: a, stats: computeStats(await predictionsByAgent(a.id)) })),
  );
